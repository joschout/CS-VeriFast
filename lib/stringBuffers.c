...
struct string_buffer *create_string_buffer_from_string(char *string)
    //@ requires [?f]string(string, ?cs);
    //@ ensures string_buffer(result, cs) &*& result != 0 &*& [f]string(string, cs);
{
    struct string_buffer *res = create_string_buffer();
    string_buffer_append_string(res, string);
    return res;
}
